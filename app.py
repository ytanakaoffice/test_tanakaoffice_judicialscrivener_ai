        ...
        if not session_rows.empty:
            if is_locked_session:
                ...
                if ptr < len(order):
                    ...
                    with col_question:
                        # 修正：indexにはデータ上のインデックスではなく、現在表示中のポインタ(ptr)を渡す
                    ...
                    selected_q_num = int(selected_q.replace("第 ", "").replace(" 問", "")) - 1
                    target_start_idx = order.index(selected_q_num) if selected_q_num in order else 0

                    if target_start_idx != ptr:
                        st.session_state.y_ptr = target_start_idx
                        st.session_state.y_answered = False
                        st.session_state.y_user_ans = None
                        st.session_state.teacher_state = "normal"
                        st.session_state.y_active_audio = None
    ...
    if not df.empty and "分野" in df.columns:
        ...
        if not cat_rows.empty:
            ...
            if ptr_c < len(order_c):
                ...
                selected_q_num_c = int(selected_q_c.replace(" 🔒[有料会員限定]", "").replace("第 ", "").replace(" 問", "")) - 1
                target_start_idx_c = order_c.index(selected_q_num_c) if selected_q_num_c in order_c else 0

                if target_start_idx_c != ptr_c:
                    st.session_state.c_ptr = target_start_idx_c
                    st.session_state.c_answered = False
                    st.session_state.c_user_ans = None
                    st.session_state.teacher_state = "normal"
                    st.session_state.c_active_audio = None
        ...
        if not current_bookmarks:
            ...
            if bookmark_rows.empty:
                ...
                if ptr_bm < len(order_bm):
                    ...
                    selected_q_num_bm = int(selected_q_bm.replace(" 🔒[有料会員限定]", "").replace("第 ", "").replace(" 問", "")) - 1
                    target_start_idx_bm = order_bm.index(selected_q_num_bm) if selected_q_num_bm in order_bm else 0

                    if target_start_idx_bm != ptr_bm:
                        st.session_state.bm_ptr = target_start_idx_bm
                        st.session_state.bm_answered = False
                        st.session_state.bm_user_ans = None
                        st.session_state.teacher_state = "normal"
                        st.session_state.bm_active_audio = None
